const inventors = [
  ...
  const dob = inventors.filter(ele =>{
...
//   console.log(fullName)
  ...
  const agesum = inventors.reduce((acc,curr)=>acc+=(curr.passed-curr.year),0)
  //console.log(agesum)
  ...
  const deGarden = parisBoulevards.filter(ele=>{
    if(ele.indexOf('de')!==-1)
        return true;
  })
  
  //console.log(deGarden)
  // 7. sort Exercise
  // Sort the people alphabetically by last name
  const sortedByLastName = people.sort((a, b) => {
    const lastNameA = a.split(", ")[1]; // Extract last name
    const lastNameB = b.split(", ")[1]; // Extract last name
